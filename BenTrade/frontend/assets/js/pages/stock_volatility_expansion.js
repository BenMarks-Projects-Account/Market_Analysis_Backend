/**
 * BenTrade — Volatility Expansion Stock Strategy Dashboard
...
window.BenTradePages.initStockVolatilityExpansion = function initStockVolatilityExpansion(rootEl) {
  var STRATEGY_ID = 'stock_volatility_expansion';
  ...
  var CACHE_ID    = 'volatilityExpansion';
  ...
  // Hydrate header
  if (titleEl)    titleEl.textContent   = (meta ? meta.name : 'Volatility Expansion') + ' — Stock Scanner';
  if (subtitleEl) subtitleEl.textContent = meta ? meta.description : 'Compression→expansion breakout entries';
  ...
  var STRATEGY_DESCRIPTION = 'Volatility Expansion detects stocks exiting tight compression ranges — Bollinger Band squeezes, narrowing ATR, or Keltner Channel contractions — right as volatility begins to expand. These compression-to-expansion transitions often precede strong directional moves, and the scanner captures entries as the breakout unfolds.';
  (function initBanner() {
    ...
    if (bannerTitle) bannerTitle.textContent = (meta ? meta.name : 'Volatility Expansion') + ' Strategy';
  ...
  function renderCandidates(payload) {
    ...
    candidates.forEach(function (row, idx) {
      try {
        ...
        console.warn('VolExpansion: card render error for candidate ' + idx, cardErr);
    ...
    if (renderErrors.length) console.warn('VolExpansion: ' + renderErrors.length + ' card render errors', renderErrors);
    ...
    candidatesEl.addEventListener('click', function (e) {
      ...
      if (action === 'data-workbench' && row) {
        ...
        console.log('[VolExpansion] Testing Workbench stub for:', tradeKey);
  ...
  function _handleReject(btn, tradeKey) {
    ...
    console.log('[VolExpansion] Rejected:', tradeKey);
  }
  ...
  async function runScan() {
    ...
    try {
      var endpoint = meta ? meta.endpoint : '/api/stocks/volatility-expansion';
      ...
      console.error('Volatility Expansion scan error:', err);
