/**
 * BenTrade — Pullback Swing Stock Strategy Dashboard
...
window.BenTradePages.initStockPullbackSwing = function initStockPullbackSwing(rootEl) {
  var STRATEGY_ID = 'stock_pullback_swing';
  ...
  // Hydrate header
  if (titleEl)    titleEl.textContent   = (meta ? meta.name : 'Pullback Swing') + ' — Stock Scanner';
  if (subtitleEl) subtitleEl.textContent = meta ? meta.description : 'Short-term dip buys in trending stocks';
  ...
  var STRATEGY_DESCRIPTION = 'Pullback Swing identifies stocks in established uptrends that have pulled back to key support levels — such as the 20 or 50-day moving average — presenting a high-probability long entry before the trend resumes. Filters include trend strength (SMA slope), pullback depth, volume confirmation, and RSI support.';
  (function initBanner() {
    ...
    if (bannerTitle) bannerTitle.textContent = (meta ? meta.name : 'Pullback Swing') + ' Strategy';
  ...
  function renderCandidates(payload) {
    ...
    candidates.forEach(function (row, idx) {
      try {
        ...
        console.warn('PullbackSwing: card render error for candidate ' + idx, cardErr);
    ...
    if (renderErrors.length) console.warn('PullbackSwing: ' + renderErrors.length + ' card render errors', renderErrors);
    candidatesEl.innerHTML = html;

    if (emptyStateEl) emptyStateEl.style.display = 'none';

    // Wire action delegation (handles ALL actions from renderFullCard + stock-analysis)
    candidatesEl.addEventListener('click', function (e) {
      ...
      if (action === 'data-workbench' && row) {
        ...
        console.log('[PullbackSwing] Testing Workbench stub for:', tradeKey);
  ...
  function _handleReject(btn, tradeKey) {
    ...
    console.log('[PullbackSwing] Rejected:', tradeKey);
  }
  ...
  async function runScan() {
    ...
    try {
      var endpoint = meta ? meta.endpoint : '/api/stocks/pullback-swing';
      ...
      console.error('Pullback Swing scan error:', err);
