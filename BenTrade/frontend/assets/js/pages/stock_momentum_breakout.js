/**
 * BenTrade — Momentum Breakout Stock Strategy Dashboard
...
window.BenTradePages.initStockMomentumBreakout = function initStockMomentumBreakout(rootEl) {
  var STRATEGY_ID = 'stock_momentum_breakout';
  ...
  // Hydrate header
  if (titleEl)    titleEl.textContent   = (meta ? meta.name : 'Momentum Breakout') + ' — Stock Scanner';
  if (subtitleEl) subtitleEl.textContent = meta ? meta.description : 'Breakout entries on volume expansion through resistance';
  ...
  var STRATEGY_DESCRIPTION = 'Momentum Breakout targets stocks breaking through key resistance levels on expanding volume — a classic signal of institutional participation. The scanner filters for price compression near resistance, relative volume surges, and trend-aligned momentum indicators to catch entries early in the breakout move.';
  (function initBanner() {
    ...
    if (bannerTitle) bannerTitle.textContent = (meta ? meta.name : 'Momentum Breakout') + ' Strategy';
  ...
  function renderCandidates(payload) {
    ...
    candidates.forEach(function (row, idx) {
      try {
        ...
        console.warn('MomentumBreakout: card render error for candidate ' + idx, cardErr);
    ...
    if (renderErrors.length) console.warn('MomentumBreakout: ' + renderErrors.length + ' card render errors', renderErrors);
    ...
    candidatesEl.addEventListener('click', function (e) {
      ...
      if (action === 'data-workbench' && row) {
        ...
        console.log('[MomentumBreakout] Testing Workbench stub for:', tradeKey);
  ...
  function _handleReject(btn, tradeKey) {
    ...
    console.log('[MomentumBreakout] Rejected:', tradeKey);
  }
  ...
  async function runScan() {
    ...
    try {
      var endpoint = meta ? meta.endpoint : '/api/stocks/momentum-breakout';
      ...
      console.error('Momentum Breakout scan error:', err);
