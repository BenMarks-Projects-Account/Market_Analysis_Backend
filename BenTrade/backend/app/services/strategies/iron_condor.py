...
class IronCondorStrategyPlugin(StrategyPlugin):
    ...
    def build_candidates(self, inputs: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # ── Penny-wing precheck thresholds (configurable via preset) ──
        # min_short_leg_mid: each short leg must have mid >= this value.
        # min_side_credit:   each side's net credit (short_mid - long_mid)
        #                    must >= this value.
        # These prechecks run BEFORE the safety ceiling so that the
        # generation cap is not wasted on structurally un-tradable condors.
        _min_short_leg_mid = self._to_float(payload.get("min_short_leg_mid"))
        if _min_short_leg_mid is None:
            _min_short_leg_mid = 0.05  # safe default
        _min_side_credit = self._to_float(payload.get("min_side_credit"))
        if _min_side_credit is None:
            _min_side_credit = 0.03  # safe default
        ...
        # ── Candidate construction diagnostic counters ──────────────────
        _ctr_total_combos = 0        # total combos evaluated (all 4-leg sets)
        _ctr_rejected_sigma = 0      # rejected by sigma distance
        _ctr_rejected_geometry = 0   # rejected by symmetry/wing geometry
        _ctr_rejected_penny_wing = 0 # rejected by penny-wing precheck
        _ctr_rejected_side_credit = 0  # rejected by side credit precheck
        _ctr_kept_for_enrichment = 0   # passed all prechecks
            ...
            for put_short in reversed([s for s in put_strikes if s < spot]):
                ...
                for call_short in [s for s in call_strikes if s > spot]:
                    ...
                    if not allow_skewed and symmetry < 0.55:
                        _ctr_rejected_geometry += 1
                        continue

                    # ── Penny-wing precheck (pre-enrichment) ──────────────
                    # Uses raw leg bid/ask from option chain contracts.
                    # Rejects candidates where either side is a "penny
                    # market" BEFORE they consume a max_candidates slot.
                    _put_long_leg = put_map.get(put_long)
                    _call_long_leg = call_map.get(call_long)
                    _ctr_total_combos += 1

                    # -- Compute per-leg mids from raw chain quotes --
                    _sp_bid_raw = self._to_float(getattr(put_short_leg, "bid", None))
                    _sp_ask_raw = self._to_float(getattr(put_short_leg, "ask", None))
                    _sc_bid_raw = self._to_float(getattr(call_short_leg, "bid", None))
                    _sc_ask_raw = self._to_float(getattr(call_short_leg, "ask", None))
                    _lp_bid_raw = self._to_float(getattr(_put_long_leg, "bid", None))
                    _lp_ask_raw = self._to_float(getattr(_put_long_leg, "ask", None))
                    _lc_bid_raw = self._to_float(getattr(_call_long_leg, "bid", None))
                    _lc_ask_raw = self._to_float(getattr(_call_long_leg, "ask", None))

                    # mid = (bid + ask) / 2; None if either is missing
                    _sp_mid = ((_sp_bid_raw + _sp_ask_raw) / 2.0
                               if _sp_bid_raw is not None and _sp_ask_raw is not None else None)
                    _sc_mid = ((_sc_bid_raw + _sc_ask_raw) / 2.0
                               if _sc_bid_raw is not None and _sc_ask_raw is not None else None)
                    _lp_mid = ((_lp_bid_raw + _lp_ask_raw) / 2.0
                               if _lp_bid_raw is not None and _lp_ask_raw is not None else None)
                    _lc_mid = ((_lc_bid_raw + _lc_ask_raw) / 2.0
                               if _lc_bid_raw is not None and _lc_ask_raw is not None else None)

                    # Gate 1: Both short legs must have mid >= min_short_leg_mid
                    _precheck_pass = True
                    if (_sp_mid is not None and _sp_mid < _min_short_leg_mid) or \
                       (_sc_mid is not None and _sc_mid < _min_short_leg_mid):
                        _ctr_rejected_penny_wing += 1
                        _precheck_pass = False
                    # Also reject if short leg quotes are completely missing
                    # (can't determine if penny wing or not — treat as garbage)
                    elif _sp_mid is None or _sc_mid is None:
                        _ctr_rejected_penny_wing += 1
                        _precheck_pass = False

                    # Gate 2: Both sides must contribute non-trivial credit
                    # put_side_credit  = short_put_mid  - long_put_mid
                    # call_side_credit = short_call_mid - long_call_mid
                    if _precheck_pass and _lp_mid is not None and _lc_mid is not None:
                        _put_side_cr = _sp_mid - _lp_mid
                        _call_side_cr = _sc_mid - _lc_mid
                        if _put_side_cr < _min_side_credit or _call_side_cr < _min_side_credit:
                            _ctr_rejected_side_credit += 1
                            _precheck_pass = False
                    elif _precheck_pass:
                        # Long leg quotes missing — can't verify credit
                        # contribution; let it through to enrichment where
                        # full DQ will catch it.
                        pass

                    if not _precheck_pass:
                        continue

                    _ctr_kept_for_enrichment += 1
        ...
        # ── Attach candidate construction diagnostics to inputs ────────
        # These are picked up by strategy_service and threaded into the
        # filter_trace → stages[candidate_construction].sub_stages dict.
        inputs["_build_sub_stages"] = {
            "total_combos_evaluated": _ctr_total_combos,
            "rejected_sigma_distance": _ctr_rejected_sigma,
            "rejected_geometry": _ctr_rejected_geometry,
            "rejected_penny_wing_precheck": _ctr_rejected_penny_wing,
            "rejected_side_credit_too_low": _ctr_rejected_side_credit,
            "kept_for_enrichment": _ctr_kept_for_enrichment,
            "thresholds": {
                "min_short_leg_mid": _min_short_leg_mid,
                "min_side_credit": _min_side_credit,
                "min_sigma_distance": min_sigma,
            },
        }
        logger.info(
            "event=ic_build_candidates total_combos=%d "
            "rejected_sigma=%d rejected_geometry=%d "
            "rejected_penny_wing=%d rejected_side_credit=%d "
            "kept=%d max_candidates=%d",
            _ctr_total_combos, _ctr_rejected_sigma,
            _ctr_rejected_geometry, _ctr_rejected_penny_wing,
            _ctr_rejected_side_credit, _ctr_kept_for_enrichment,
            max_candidates,
        )
    ...
    def enrich(self, candidates: list[dict[str, Any]], inputs: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # Apply expected-fill model to each enriched IC trade.  IC always
        # uses mid-based net_credit; spread_bid is the natural price.
        # apply_expected_fill will derive spread_mid from net_credit and
        # spread_natural from spread_bid for credit strategies.
        for trade in out:
    ...
    def evaluate(self, trade: dict[str, Any]) -> tuple[bool, list[str]]:
        ...
        # Fill-aware: prefer ror_fill when available, fall back to mid
        min_ror = safe_float(request_payload.get("min_ror"))
        if min_ror is None:
            min_ror = 0.12
        _ror = safe_float(trade.get("ror_fill")) or safe_float(trade.get("return_on_risk"))
        if (_ror or 0.0) < min_ror:
